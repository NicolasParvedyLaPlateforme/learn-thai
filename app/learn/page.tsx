      ...
      {mounted && (
        <div className="hidden md:flex flex-row w-full max-w-[100rem] mx-auto px-6 lg:px-8 py-8 items-start relative min-h-screen">
          ...
          <div className="w-[20rem] flex-shrink-0 relative hidden xl:block z-40 pl-8 ml-2 border-l-2 border-slate-100/60 pb-16">
            <div className="w-full relative flex flex-col gap-4 group mt-2">
              ...
              <div className="flex flex-col gap-3 pb-6 w-full">
                {UNITS.map((u, i) => {
                  ...
                  const unitLessons = data.lessons.slice(u.startIndex, u.endIndex);
                  const maxLevelsInUnit = unitLessons.length * 10;
                  const completedLevelsInUnit = mounted ? unitLessons.reduce((acc, l) => acc + (lessonLevels[l.id] || 0), 0) : 0;
                  const progressPercent = mounted ? (completedLevelsInUnit / maxLevelsInUnit) * 100 : 0;
                  const completedLessonsCount = mounted ? unitLessons.filter(l => (lessonLevels[l.id] || 0) >= 10).length : 0;
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div className="flex flex-col min-w-0 pr-2">
                            <h3 className="font-bold text-[15px] text-slate-800 truncate leading-tight mb-0.5">{language === 'en' ? u.titleEn : u.title}</h3>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col justify-center min-w-0 overflow-hidden pr-2">
                        <h3 className="font-bold text-[14px] truncate text-slate-400 group-hover/btn:text-slate-500 transition-colors">{language === 'en' ? u.titleEn : u.title}</h3>
                      </div>
                    </button>
                  )
                })}
                      ...
                      <div className="mb-2">
                        ...
                        <div>
                           ...
                           <div className="flex flex-nowrap overflow-x-auto hide-scrollbar gap-2 pb-2">
                               {selectedLesson.lesson.words.slice(0, 3).map(w => (
